...
void threadFunction(int i,int len,int number) {
    ++numberOfThreads;
    //std::cout << number << std::endl;
    int offsetX = 2;
    int offsetY = -50;
    for (int j = 0; j < len; j++){
        if (iterClicked > 0)break;
        if (buffer[i][j].r == 255 && buffer[i][j].g == 0 && buffer[i][j].b == 0) {
            //std::cout << "equal" << std::endl;
            ++iterClicked;
            if (iterClicked == 1) {
                mouseClick(int(float(j + offsetX + CAPTURE_X_OFFSET) / 1920 * 65535), int(float(buffer.size() - (i + offsetY + CAPTURE_Y_OFFSET)) / 1080 * 65535));
...
int main() {
    ...
    std::thread threadRow[CAPTURE_HEIGHT];

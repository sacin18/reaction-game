...
__device__ bool myGlobalBoolVar = false;
__device__ int iterClicked = 0;
__device__ int numberOfCudaThreads = 0;

#define PIPE_NAME "\\\\.\\pipe\\smth"
...
__global__ void scanKernel(pixel* grid, int gridHeight, int gridWidth, int* sol, int numberOfThreads) {
    //int offsetX = LINE_X_OFFSET;
    //float offsetY = LINE_Y_OFFSET;
    /*
    for (int i = 0; i < gridHeight; i++) {
    ...
    */
    int id = threadIdx.x;
    int sharestart = id * gridHeight / numberOfThreads;
    int shareend = min(id * gridHeight / numberOfThreads + (gridHeight / numberOfThreads) - 1, gridHeight - 1);
...
cudaError_t targetWithCuda(pixel* grid, unsigned int height, unsigned int width, int* sol, unsigned int numberOfThreads);
...
int main() {
    ...
    if (WaitNamedPipe(TEXT(PIPE_NAME), NMPWAIT_WAIT_FOREVER) == 0) {
        printf("WaitNamedPipe failed with error %d\n", GetLastError());
        return 0;
    }

    // Create the named pipe file handle
    if ((PipeHandle = CreateFile(TEXT(PIPE_NAME),
    ...
    for (int iter = 1; iter <= TOTAL_ITERATIONS; iter++) {
        ...
        cudaError_t cudaStatus = targetWithCuda(buffer1D, bufferHeight, bufferWidth, sol, TOTAL_THREADS);
        ...
        if (WriteFile(PipeHandle, std::to_string(int(float(sol[0])/bufferWidth*700)+1).c_str(), 14, &BytesWritten, NULL) == 0) {
...
cudaError_t targetWithCuda(pixel* grid, unsigned int height, unsigned int width, int *sol, unsigned int numberOfThreads) {
    ...
    unsigned int size = numberOfThreads;
    iterClicked = 0;
    ...
    scanKernel <<<1, size >>> (cudaGrid, height, width, cudaSol, numberOfThreads);
